...
?>
    ...
    <?php
         ...
         $query="SELECT * from complaint_details WHERE complaint_hostel_name='$hostel_names' AND complaint_status='approved' ";
    $approved_complaint_list_query=mysqli_query($connection,$query);
         if(!$approved_complaint_list_query)
             die("QUERY FAILED".mysqli_error($connection));
         
         while($row=mysqli_fetch_assoc($approved_complaint_list_query))
         {
     ...
     <!--repeating the same code due to the screen size-->
       <div class="container-fluid" id="for_mobiles">
         ...
         <?php
          ...
          $count=1;
         $hostel_names=$_SESSION['hostel_name'];
         $query="SELECT * from complaint_details WHERE complaint_hostel_name='$hostel_names' AND complaint_status='approved' ORDER BY complaint_id DESC ";
    $approved_complaint_list_query=mysqli_query($connection,$query);
         if(!$approved_complaint_list_query)
             die("QUERY FAILED".mysqli_error($connection));
